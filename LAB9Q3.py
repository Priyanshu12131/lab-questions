...
is_symmetric = (matrix[i][j] == matrix[j][i] for i in range(N) for j in range(N))
...
is_upper_triangular = (matrix[i][j] == 0 for i in range(1, N) for j in range(i))
is_lower_triangular = (matrix[i][j] == 0 for i in range(N) for j in range(i+1, N))
